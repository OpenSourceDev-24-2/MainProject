...
import com.mysite.sbb.payload.ApiResponse;
...
public class QuestionApiController {
    ...
    public ResponseEntity<ApiResponse> deleteQuestion(@PathVariable Long id, Principal principal) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
        ...
        boolean isDeleted = questionService.deleteQuestion(id, currentUsername);

        if (isDeleted) {
            return ResponseEntity.ok(new ApiResponse(true, "게시글이 성공적으로 삭제되었습니다."));
        } else {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new ApiResponse(false, "게시글 삭제 권한이 없습니다."));
        }
    }
    ...
    public ResponseEntity<ApiResponse> updateQuestion(@PathVariable Long id,
        ...
        String currentUsername = principal.getName();
        boolean isUpdated = questionService.updateQuestion(id, questionForm, currentUsername);

        if (isUpdated) {
            return ResponseEntity.ok(new ApiResponse(true, "게시글이 성공적으로 수정되었습니다."));
        } else {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new ApiResponse(false, "게시글 수정 권한이 없습니다."));
        }
    }
}
